...
async def dry(message: types.Message):
    await message.answer("Сушка: кардио утром, дефицит -500 калорий, вода 3л/день.")

@dp.message_handler(commands=['nutrition'])
async def nutrition(message: types.Message):
    await message.answer("Питание: белок 2г/кг, угли до 100г, жиры умеренно, 5 приёмов пищи.")
